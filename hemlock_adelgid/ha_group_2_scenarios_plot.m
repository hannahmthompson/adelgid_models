...
% Plotting select varied initial condition scenarios
...
% initial conditions to plot results with
init_test = [0.20  2.1
             0.20  2.2
             0.20  2.3
             0.20  3.8
...
% loop through initial condition scenarios
for q=1:length(init_test)
    
    % pull intial conditions from init_test matrix
    h_0 = init_test(q, 1);
    a_0 = init_test(q, 2);
    ...
    % create a function representing 0.1 proportion tips alive
    % threshold (a horizontal line- H(t)=0.1)
    ...
    ax.FontSize = 16;
    % tips alive in green
    % threshold proportion tips alive with dotted line,
    % model results as solid line
    ...
    ylim([0, 1])
    % print initial conditions on plot
    text(0.1, 0.92, sprintf('H(0)= %1.2f', h_0), 'FontSize', 14) 
    ...
    filename = ['ha_group_2_H_' num2str(h_0 * 100) '_A_' num2str(a_0 * 10) '.jpg'];
    f = gcf;
